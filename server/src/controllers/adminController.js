...
class AdminController {
    static async blockUser(req, res) {
        try {
            ...
            res.status(200).json({success: true,message: 'User blocked successfully',data: {id: user._id,name: user.name,email: user.email,phone: user.phone,isBlocked: user.isBlocked}});
        } catch (error) {
    ...
    static async unblockUser(req, res) {
        try {
            ...
            res.status(200).json({success: true,message: 'User unblocked successfully',data: {id: user._id,name: user.name,email: user.email,phone: user.phone,isBlocked: user.isBlocked}});
        } catch (error) {
    ...
    static async verifyOperator(req, res) {
        try {
            ...
            res.status(500).json({success: false,message: 'Failed to verify operator',error: error.message});
        }
    }
    ...
    static async cancelTrip(req, res) {
        try {
            ...
            if (!reason) {
                return res.status(400).json({success: false,message: 'Cancellation reason is required',data: {}});
            }
