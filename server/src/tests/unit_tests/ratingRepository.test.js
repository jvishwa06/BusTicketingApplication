...
describe('RatingRepository', () => {
  ...
  describe('getAverageRatingForBus', () => {
    it('should get average rating for a bus successfully', async () => {
      ...
      Rating.aggregate.mockResolvedValue(mockResult);

      const result = await RatingRepository.getAverageRatingForBus(mockBusId);

      expect(mongoose.Types.ObjectId).toHaveBeenCalledWith(mockBusId);
      expect(Rating.aggregate).toHaveBeenCalled();
      expect(result).toEqual(mockResult[0]);
    });

    it('should return default values when no ratings exist for bus', async () => {
      const mockBusId = 'bus123';
      ...
      expect(result).toEqual({ _id: mockBusId, averageRating: 0, count: 0 });
    });

    it('should throw an error when calculating average rating fails', async () => {
      const mockBusId = 'bus123';
      const error = new Error('Aggregation failed');
      
      Rating.aggregate.mockRejectedValue(error);

      await expect(RatingRepository.getAverageRatingForBus(mockBusId)).rejects.toThrow(error);
      expect(appLogger.error).toHaveBeenCalledWith(`Error calculating average rating for bus ${mockBusId}: ${error.message}`);
    });
  });

  describe('getAverageRatingForTrip', () => {
    it('should get average rating for a trip successfully', async () => {
      ...
      const result = await RatingRepository.getAverageRatingForTrip(mockTripId);

      expect(mongoose.Types.ObjectId).toHaveBeenCalledWith(mockTripId);
      expect(Rating.aggregate).toHaveBeenCalled();
      expect(result).toEqual(mockResult[0]);
    });

    it('should return default values when no ratings exist for trip', async () => {
      const mockTripId = 'trip123';
      ...
      expect(result).toEqual({ _id: mockTripId, averageRating: 0, count: 0 });
    });
    ...
    it('should throw an error when calculating average rating fails', async () => {
      ...
      await expect(RatingRepository.getAverageRatingForTrip(mockTripId)).rejects.toThrow(error);
      expect(appLogger.error).toHaveBeenCalledWith(`Error calculating average rating for trip ${mockTripId}: ${error.message}`);
    });
  });
